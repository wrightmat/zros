...
game_meta:register_event("on_command_released", function(game, command)
  local hero = game:get_hero()
...
hero_meta:register_event("is_condition_active", function(hero, condition)
  if hero.condition ~= nil then
...
hero_meta:register_event("start_confusion", function(hero, delay)
  ...
  if not bAlreadyConfused then
    for key, value in pairs(aDirectionPressed) do
      if value then game:simulate_command_pressed(key) end
    end
  end
end)

hero_meta:register_event("start_frozen", function(hero, delay)
  if hero:is_condition_active('frozen') then
    return
  end
...
hero_meta:register_event("start electrocution", function(hero, delay)
...
hero_meta:register_event("start_poison", function(hero, damage, delay, max_iteration)
...
hero_meta:register_event("start_slow", function(hero, delay)
...
hero_meta:register_event("start_cursed", function(hero, delay)
...
hero_meta:register_event("stop_confusion", function(hero)
  local aDirectionPressed = {
    right = {"left", false},
    left = {"right", false},
    up = {"down", false},
...
hero_meta:register_event("stop_frozen", function(hero)
...
hero_meta:register_event("stop_electrocution", function(hero)
...
hero_meta:register_event("stop_slow", function(hero)
  if hero:is_condition_active('slow') and condition_manager.timers['slow'] ~= nil then
    condition_manager.timers['slow']:stop()
  end
...
hero_meta:register_event("stop_cursed", function(hero)
  if hero:is_condition_active('cursed') and condition_manager.timers['cursed'] ~= nil then
    condition_manager.timers['cursed']:stop()
  end
