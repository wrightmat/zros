...
function swimming_bar:new(game)
  local object = {}
...
function swimming_bar:initialize(game)
  self.game = game
  ...
  if magic ~= self.magic_displayed then
    ...
    if increment ~= 0 then
      ...
      if increment == 1 and (magic - self.magic_displayed) % 3 == 1 then
	      -- sol.audio.play_sound("common/swimming_bar/" .. self.sound_to_play)
      end
    end
  end
...
function swimming_bar:rebuild_surface()
  self.surface:clear()
  local position = 6
...
function swimming_bar:set_dst_position(x, y)
  self.dst_x = x
  self.dst_y = y
end

function swimming_bar:on_draw(dst_surface)
  -- Is there a swimming bar to show?
